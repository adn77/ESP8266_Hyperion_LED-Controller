/*------------------------------------------------*/
/*Configuration type*/

#define CONFIG_ENABLE_WEBCONFIG 1

//Replaces all values in the webconfig with the ConfigStatic.h values when CONFIG_ENABLE_WEBCONFIG is enabled
//#define CONFIG_OVERWRITE_WEBCONFIG 1

#define SWVERSION "hyperionrgb_ws2812_17102305"
/*------------------------------------------------*/
/*Logging level*/

#define LOGLEVEL LOG_LEVEL_INFOS
//#define LOGLEVEL LOG_LEVEL_DEBUG
//#define LOGLEVEL LOG_LEVEL_VERBOSE

/*------------------------------------------------*/
/*Hardware library support*/
//#define HW_FASTLED 1
#define HW_NEOPIXEL 1

/*------------------------------------------------*/
/*Main static configuration*/
//This cannot be changed via web inteface

#ifdef HW_FASTLED
  #define CONFIG_LED_SPI_CHIPSET WS2801 //Comment out for clockless
  //#define CONFIG_LED_CLOCKLESS_CHIPSET WS2812B //Comment in for clockless
  //#define FASTLED_ALLOW_INTERRUPTS 0 //Comment in if clockless stripe (ex. WS2812B) is flickering
  //#define CONFIG_LED_PWM 1 //Comment in if PWM Stripe

  #define CONFIG_LED_DATAPIN 2 //Comment out for PWM
  #define CONFIG_LED_CLOCKPIN 0 //Comment out for clockless / PWM
  //#define CONFIG_LED_PWM_RED D1 //Comment in for PWM
  //#define CONFIG_LED_PWM_GREEN D2 //Comment in for PWM
  //#define CONFIG_LED_PWM_BLUE D3 //Comment in for PWM

  //Pin order, see FastLED doc. NodeMCU should work with FASTLED_ESP8266_RAW_PIN_ORDER
  #define FASTLED_ESP8266_RAW_PIN_ORDER
  //#define FASTLED_ESP8266_NODEMCU_PIN_ORDER
  //#define FASTLED_ESP8266_D1_PIN_ORDER

  #define CONFIG_LED_COLOR_ORDER RGB
#elif HW_NEOPIXEL
  #define CONFIG_LED_DATAPIN 2
  #define CONFIG_NEO_METHOD   NeoEsp8266Uart800KbpsMethod //one in NeoEsp8266DMA800KbpsMethod, NeoEsp8266Uart800KbpsMethod, NeoEsp8266AsyncUart800KbpsMethod, NeoEsp8266BitBang800KbpsMethod (see https://github.com/Makuna/NeoPixelBus/wiki/NeoPixelBus-object)
  #define CONFIG_NEO_FEATURE  NeoGrbFeature               //one in NeoGrbFeature, NeoGrbwFeature, NeoRgbFeature, NeoRgbwFeature, NeoBrgFeature, NeoRbgFeature
#endif

#define CONFIG_LED_COUNT 15

//OFF, HYPERION_UDP, STATIC_COLOR, RAINBOW, FIRE2012
#define CONFIG_LED_STANDARD_MODE HYPERION_UDP
#define CONFIG_LED_HYPERION_AUTOSWITCH true 
#define CONFIG_LED_STANDARD_MODE_TIMEOUT_MS 5000 

/*------------------------------------------------*/
/*Main configuration*/
//You should leave it empty and override it via the web interface on port 80
#define CONFIG_WIFI_SSID ""
#define CONFIG_WIFI_PASSWORD ""

//#define CONFIG_WIFI_STATIC_IP 1
//const byte CONFIG_WIFI_IP[] = {192, 168, 0, 100};
//const byte CONFIG_WIFI_SUBNET[] = {255, 255, 255, 0};
//const byte CONFIG_WIFI_DNS[] = {192, 168, 0, 1};

#define CONFIG_WIFI_HOSTNAME "hyperion"

#define CONFIG_PORT_JSON_SERVER 19444
#define CONFIG_PORT_UDP_LED 19446
#define CONFIG_MAX_JSON_CLIENTS 5

/*------------------------------------------------*/
/* Enable Web OTA 
 *  
 *  transmitted updateversion string will look like:
 *    ssid=CONFIG_WIFI_SSID%SWVERSION
*/
#define CONFIG_WEB_OTA 1
#define CONFIG_WEB_OTA_UPDATESERVER "loetzimmer.de"
#define CONFIG_WEB_OTA_UPDATEPORT 80
#define CONFIG_WEB_OTA_UPDATEFILE "/esp8266/index.php"
